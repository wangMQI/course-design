...
import com.stephen.coursedesign.config.FileUploadConfig;
import com.stephen.coursedesign.entity.FileRecord;
import com.stephen.coursedesign.entity.FileZoneRecord;
import com.stephen.coursedesign.entity.mapper.FileRecordMapper;
import com.stephen.coursedesign.service.IFileRecordService;
import com.stephen.coursedesign.service.IFileZoneRecordService;
import com.stephen.coursedesign.util.fileUpload.FileHandleUtil;
import com.stephen.coursedesign.comment.util.IPUtils;
import com.stephen.coursedesign.comment.util.IdWorker;
...
//    private IFileDownloadLogService fileDownloadLogService;
    ...
    @Override
    public ResultV upload(HttpServletRequest request, Integer uploadType, Integer storageYear) {
...
//                    fileRecorddb.setUploadCount(fileRecorddb.getUploadCount()+1);
    ...
    @Override
    public ResultV zoneUpload(HttpServletRequest request,String contentType, FileZoneRecord fileZoneRecord) {
        MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
        Map<String, MultipartFile> files = multipartRequest.getFileMap();
        for (Map.Entry<String, MultipartFile> entry : files.entrySet()) {
            MultipartFile multipartFile = entry.getValue();
...
//                        }
    ...
    @Override
    public ResultV md5Check(FileZoneRecord fileZoneRecord, Integer checkType,String contentType,HttpServletRequest request) {
        if(checkType==1){//校验文件
//            if(contentType==null||!isFileter(contentType)){
//                return new Result(ResultCode.FILENOTSUPPORT);
//            }
            ...
            return fileRecordb!=null&&fileRecordb.getIsMerge()==1?new ResultV(ResultCode.FILEUPLOADED,fileRecordb):renderError("请选择文件上传");
        }else{
    ...
    @Override
    public ResultV mergeZoneFile(String totalmd5,HttpServletRequest request) {
        //查询所有的分片文件
    ...
    @Override
    public ResultV delZoneFile(String totalmd5) {
        return null;
    }

    @Override
    public ResultV delFile(String fileId) {
        return null;
    }
...
//    }
    ...
    public ResultV renderError(String msg){
        return new ResultV(ResultCode.SERVER_ERROR,msg );
    }
    public ResultV renderDataSuccess(Object obj){
        return new ResultV(ResultCode.SUCCESS,obj);
    }
    //
